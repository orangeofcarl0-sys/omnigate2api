...
func TestExtractToolCallsFenced(t *testing.T) {
	...
	// 回合语义在首个 tool_call 处结束：调用后的文本（多为编造结果）不进入 rest。
	if strings.Contains(rest, "tool_call") || !strings.Contains(rest, "先分析") {
		t.Fatalf("rest=%q", rest)
	}
	if strings.Contains(rest, "然后结束") {
		t.Fatalf("post-call text should be truncated, rest=%q", rest)
	}
}
